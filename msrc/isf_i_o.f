...
c     Chars 7-15: event ID
      if (partline(evid,line,7,9) .eq. 0) then
...
c     Char 16 or Char 15: must be a space
...
c     Chars 16/17-80: geographic region if there
      if(line(16:16) .eq. ' ') then
...
c     Chars 1-5: the word 'Event'. Chars 7-15: event ID.
      ...
      write (file,'()')
      write (file,'(''Event '',a9,$)') evid

c     Chars 17-80: geographic region if given.
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      integer unit
      character line*155,com*50

      character uppcas*30,chgcas*30
     ...
     +          magind*1,picktype*1
      ...
      if(phas1.eq.'pwP     ' .and. phas2.eq.'pP      ') then
         phas2 = phas1
      endif
      if(phas2.ne.'        ') phas1=phas2
      if(phas1.eq.'        ') phas1='tx'
      ...
      slow = ISF_NULL
      if(line(75:79).ne.'     ') read (line(75:79),'(f5.2)') slow
      slowres = ISF_NULL
      ...
      timedef = '_'
      if(uppcas(line(88:88)).eq.'T') timedef = 'T'
      if(line(88:88).eq.'m') timedef = 'x'
      azimdef = '_'
      if(uppcas(line(89:89)).eq.'A') azimdef = 'A'
      if(line(89:89).eq.'m') azimdef = 'x'
      slowdef = '_'
      if(uppcas(line(90:90)).eq.'S') slowdef = 'S'
